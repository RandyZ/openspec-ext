...
- **i18n**: All user-facing strings use `t('key')` from `src/i18n/`. Locale files: `src/i18n/locales/en.json` (English, default) and `zh-cn.json` (Chinese). Extension detects locale from `vscode.env.language`; webview from `document.documentElement.lang`.
- **Adapter priority**: VS Code Copilot Chat > Cursor agent CLI > Clipboard. The Copilot adapter uses `workbench.action.chat.open` with `query` param to pre-fill Chat input directly. Install `github.copilot-chat` extension for it to appear in the executor dropdown.
- **Workflow commands**: All workflow buttons (Apply, Verify, Continue, FF, etc.) send simple `/opsx:<action> <change-name>` commands — no verbose prompts. OpenSpec skills handle context loading internally.
