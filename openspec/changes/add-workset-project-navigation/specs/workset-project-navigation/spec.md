...
The Workset navigation scene SHALL distinguish selectable Project members from registered Store members, and SHALL never make a Store member a Project selection target.
...
- **THEN** Store members MUST be labeled `Planning Store` and rendered read-only
- **AND** only non-Store Project members MUST have selection controls
...
- **THEN** the Workset scene MUST show best-effort repository identity and branch metadata
...
- **THEN** the Workset scene MUST show a clear empty state
